...
reconizer=InceptionResnetV1(pretrained='vggface2').eval().to(device=device)
...
def face_recognition():
    ...
    global faces_img,faces,latest_frame,thread_stop_flag
        ...
        if latest_frame is not None:
            ...
            with lock:
                ...
                for i,face in enumerate(faces):
                    ...
                    if x2 > x1 and y2 > y1:
                        faces_img.append(frame_copy[y1:y2, x1:x2])
                
                if len(faces_img)>0:
                    resized_faces=[cv2.resize(face,(150,150)) for face in faces_img if face.size>0]
...
def read_display():
    ...
    camera=cv2.VideoCapture("rtsp://admin:dodmane%407854@192.168.0.115:554/cam/realmonitor?channel=2&subtype=0")
